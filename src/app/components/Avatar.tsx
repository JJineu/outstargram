  ...
  return (
    <div className={getContainerStyle(size, highlight)}>
      ...
      <img
        ...
        ${getImageSizeStype(size).image}`}
        alt="user profile"
        src={image ?? undefined}
        referrerPolicy="no-referrer" // x box issue
      />
    </div>
  );
}
...
function getContainerStyle(size: AvatarSize, highlight: boolean): string {
  ...
  const highlightStyle = highlight
    ...
    : "";
  const { container } = getImageSizeStype(size);
...
function getImageSizeStype(size: AvatarSize): ImageSizeStyle {
  switch (size) {
    case "small":
